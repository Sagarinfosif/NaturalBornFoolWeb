		   ...
		   <!-- /.box -->
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><a href="<?= site_url();?>/Sounds/addSound" style="font-size: 14px;" class="btn btn-block btn-success btn-xs">Add Sound</a></h3>
					</div>
						...
						<table id="example1" class="table table-bordered table-striped">
							...
							<?php $i = 1; foreach($details as $data){ ?>
								<tr>
									...
									<td><?php  echo $data['soundCount'];?></td>
									<td>
										<?php if($data['addedby'] == '1'){ echo 'Admin';}else{ 
										$userData = $this->db->get_where('users',array('id'=>$data['userId']))->row_array(); echo $userData['username']; }?>	
									</td>
									<td>
										<?php
										if (empty($data['soundImg'])) {?>
										<img src="<?php echo base_url()."uploads/no_image_available.png";?>" style="width: 50px;height: 50px;">
										<?php } else{ ?>
										<img src="<?php echo base_url().$data['soundImg'];?>" style="width: 50px;height: 50px;">
									<?php } ?>
									</td>
									<td>
											...
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													<li role="presentation">
														<a role="menuitem" tabindex="-1" href="<?= site_url()?>/Sounds/viewVideo/<?= $data['id'];?>">View Video</a>
													</li>
													<li role="presentation">
														<a role="menuitem" tabindex="-1" href="<?= site_url()?>/Sounds/edit/<?= $data['id'];?>">Edit</a>
													</li>
													<li>
														<a role="menuitem" tabindex="-1" href="<?= site_url()?>/Sounds/delete/<?= $data['id'];?>">Delete
											            </a>
													</li>
            ...
            if(data ==''){
                ...
                for(var i = 0; i < data.length; i++) {
                	...
                	if(comment.soundImg == ""){
                    	...
                    	var img = '<img src="<?php echo base_url().$data['soundImg'];?>" style="width: 50px;height: 50px;">'
                    }
                	var snd = '<audio controls src="<?php echo base_url().$data['sound'];?>"></audio>';
           			html += '<tr><td>' + (i+1) + '</td><td>' + comment.title + '</td><td>' + comment.type + '</td><td>' + snd + 
                    '</td><td>' + comment.soundCount + '</td><td>' + img + 
