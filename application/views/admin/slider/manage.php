		   ...
		   <!-- /.box -->
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><a href="<?= site_url();?>/Slider/add" style="font-size: 14px;" class="btn btn-block btn-success btn-xs">Add Slider</a></h3>
					</div>
											...
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													<li role="presentation">
														<a role="menuitem" tabindex="-1" href="<?= site_url()?>/Slider/edit/<?= $data['id'];?>">Edit</a>
													</li>
													<li>
														<a role="menuitem" tabindex="-1" onclick="return confirm(&quot;Once you've deleted 	this account, the Slider (<?php  echo $data['title'];?>) will no longer be able to log in to the site or apps. This action cannot be undone.&quot;);" href="<?= site_url()?>/Slider/delete/<?= $data['id'];?>">Delete
											            </a>
													</li>
													<li role="presentation" class="divider"></li>	
											        <li role="presentation">
														<a role="menuitem" onclick="return confirm(<?php  echo $data['title'];?> - Do you want <?php if($data['status'] == 'Approved'){ echo "Decline"; } else{ echo "Approve";}?> this sub Admin&quot;);" tabindex="-1" href="<?= site_url()?>/Slider/status/<?= $data['id'];?>">
            ...
            if(data ==''){
                ...
                for(var i = 0; i < data.length; i++) {
                	...
                	if(comment.image == ""){
                    	...
                    	var img = '<img src="<?php echo base_url().$data['image'];?>" style="width: 50px;height: 50px;">';
                    }
           			html += '<tr><td>' + (i+1) + '</td><td>' + comment.title + '</td><td>' + img + '</td><td><span class="label label-success">' + comment.status + 
                    '</span></td><td>' + comment.created + '</td><td>' + comment.updated + 
