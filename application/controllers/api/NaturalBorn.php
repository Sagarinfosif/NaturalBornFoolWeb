...
defined('BASEPATH') OR exit('No direct script access allowed');
	USE Aws\S3\S3Client;
class NaturalBorn extends CI_Controller {
	public function __construct(){
		...
		date_default_timezone_set('Asia/Kolkata');
		 ...
		 public function uploadVideos(){
		   ...
		   $data['soundId']  = $sound;
			 ...
			 $data['videolength']  = $this->input->post('videolength');
		   $data['status']  = '3';
		   $data['created'] = date('Y-m-d H:i:s');
		       ...
		       //*******************//
					 ...
					 if(!empty($url)){
						 $data['videoPath'] = 'http://d29vf2dbcha7zq.cloudfront.net/'.$_FILES['videoPath']['name'];
					   $data['downloadPath'] = 'http://d29vf2dbcha7zq.cloudfront.net/'.$_FILES['videoPath']['name'];
					 }
					 else{
						 $data['videoPath'] = '';
					   $data['downloadPath'] = '';
			 ...
			 $upload12 = $s3->upload($bucket, $_FILES['imageThumb']['name'], fopen($_FILES['imageThumb']['tmp_name'], 'rb'), 'public-read');
			 $url12 = $upload12->get('ObjectURL');
			 if(!empty($url12)){
				 $data['imageThumb'] = 'http://d29vf2dbcha7zq.cloudfront.net/'.$_FILES['imageThumb']['name'];;
			 }
			 else{
				 $data['imageThumb'] = '';
			 }
		   ...
		   if(!empty($insert)){
			 ...
			 if(!empty($this->input->post('hashTag'))){
				$updateHash['hashTag'] = $this->hastTagIds($this->input->post('hashTag'),$this->input->post('userId'),$vIDs);
				$this->Common_Model->update('userVideos',$updateHash,'id',$vIDs);
			}
	 ...
	 public function uploadVideosWithWatermark(){
	   ...
	   $dir_name = "var/www/html/application/controllers/api/";
	   $uploadPath = "uploads/";
	   $duetUrl = "";

	   // $uploadPath = "";

	   // $tmpName =$uploadPath . $_FILES['videoPath']['tmp_name'];
	   ...
	   // $basenameTemp = basename($tmpName,".mp4");
	   ...
	   // $videoMainPath = "uploads/abc_main.mp4";
	   // $watermarkPath = "uploads/watermark.gif";
	   // $watermarkPath = "https://cancremedia.s3.us-east-2.amazonaws.com/watermark/watermark.gif";
	   // $usernameWatermark = "testuser";
	   // $downloadPath = $basenamepath + "_download.mp4";

	   // $command_compress  =  "ffmpeg -i " . $tmpName . " -vcodec libx264 -crf 28 ".$compressvideo." -y";
	   $command_compress  =  "ffmpeg -i " . $tmpName . " -vcodec libx264 -crf 28 ".$compressVideo;
	   // $command_compress  =  "ffmpeg -i " . $tmpName . " ".$compressVideo;
	   // $command_compress = str_replace("'", "", $command_compress);
	   //   @exec($command_compress, $output, $retval);

	   shell_exec($command_compress);

	   // file_put_contents( 'uploads/file.txt', "com");

	   // file_put_contents( 'uploads/file.txt',"command = " . $output_compress );
	   $downloadPath ="";
	   // $change_size =  "ffmpeg -i ". $compressvideo. " -vf scale=".$width_video.":".$height_video. " "  .$videoMainPath. " -y";
	   $change_size =  "ffmpeg -i ".$compressVideo." -vf scale=".$width_video.":".$height_video. " ".$videoMainPath;
	   // $change_size = str_replace("'", "", $change_size);
	   // @exec($change_size, $output, $retval);
	   shell_exec($change_size);
	   // $videoMainPath = $tmpName;
	   if($isDuet == "true"){
	   ...
	   $downloadPath = $this->addWaterMarkPrivateNew($videoMainPath,$watermarkPath,$usernameWater,$basenamepath);
	   ...
	   $url = $upload->get('ObjectURL');
	   if(!empty($url)){
	     $data['downloadPath'] = 'http://d29vf2dbcha7zq.cloudfront.net/'.basename($downloadPath);
	   }
	   else{
	     $data['downloadPath'] = '';
	   }
	  ...
	  function addWaterMarkPrivateNew($video_file,$watermark,$username,$basename_file){
	...
	// -fill white    -annotate +5+0 'username'   uploads/anno_splice2.gif
	   ...
	   $new_watermark = $this->addtextImageNew($new_watermark,$watermark,$text);
	 ...
	 function addtextImageNew($gifpath,$watermarkpath,$textwrite){
	...
	public function bucketTest(){
		...
		die;
		$url = $upload->get('ObjectURL');
		//'http://d29vf2dbcha7zq.cloudfront.net/'.$_FILES['videoPath']['name'];
	}

	public function matchVerificationToken(){
		$checkOtp = $this->db->get_where('users',array('phone' => $this->input->post('phone'),'loginOtp' => $this->input->post('otp')))->row_array();
		if(!empty($checkOtp)){
			$userDetails = $this->db->get_where('users',array('id' => $checkOtp['id']))->row_array();
			if(empty($userDetails['image'])){
				$userDetails['image'] =  base_url().'uploads/no_image_available.png';
			}

			$message['success'] = '1';
			$message['message'] = 'OTP match Successfully';
			$message['details'] =$userDetails;
		}
		else{
			$message['success'] = '0';
			$message['message'] = 'sorry your verification OTP does not match!';
		}
		echo json_encode($message);
	}

	public function resendOTP(){
		$checkOtp = $this->db->get_where('users',array('phone' => $this->input->post('phone')))->row_array();
		if(!empty($checkOtp)){
			$sendOtp = rand(1000,9999);
			$datas['loginOtp'] = $sendOtp;
			$update = $this->Common_Model->update('users',$datas,'id',$checkOtp['id']);

			$mobile = $this->input->post('phone');
			$created = date('Y-m-d+H:s:i');
			$url = "https://api.msg91.com/api/v5/otp?authkey=346870A5GnDowb5fab687dP1&template_id=5fabb934c00b37344b06d88b&mobile=$mobile&invisible=1&otp=$sendOtp";
			$ch = curl_init();
			...
			$message['success'] = '1';
			$message['message'] = 'OTP sent your phone number';
			$message['details'] =$userDetails;
		}
		else{
			$message['success'] = '0';
	...
	public function loginWithPhone(){
		$checkLogin = $this->db->get_where('users',array('phone' => $this->input->post('phone')))->row_array();
		if(!empty($checkLogin)){
			$otp = rand(1000,9999);
			$datas = array('reg_id' => $this->input->post('reg_id'),'device_type' => $this->input->post('device_type'),'loginOtp' => $otp);
			...
			$update = $this->Common_Model->update('users',$datas,'id',$checkLogin['id']);
			$userDetails = $this->db->get_where('users',array('id' => $checkLogin['id']))->row_array();
			if(empty($userDetails['image'])){
				$userDetails['image'] =  base_url().'uploads/no_image_available.png';
			}
			$message = array(
				'success'=>'1',
				'message' => 'OTP sent your phone number',
				'details' => $userDetails
			);
		}
		else{
			...
			if(!empty($insert)){
				...
				$message = array('success' => '1', 'message' => 'OTP sent your phone number', 'details' => $userDetails);
			}
		}
		echo json_encode($message);
	}
	...
	public function uploadVideospehlachlrhic(){
		require APPPATH.'/libraries/vendor/autoload.php';
		// $ffmpeg = FFMpeg\FFMpeg::create(array(
				//     'ffmpeg.binaries'  => ffmpeg_lib,
				//     'ffprobe.binaries' => ffprobe_lib,
		...
		// $videoMainPath = "uploads/abc_main.mp4";
		// $watermarkPath = "uploads/watermark.gif";
		// $watermarkPath = "https://Cinemaflixmedia.s3.us-east-2.amazonaws.com/watermark/watermark.gif";
		// $usernameWatermark = "testuser";
		// $downloadPath = $basenamepath + "_download.mp4";
		...
		if($isDuet == "true"){
			...
			$videoMainPath = $dueturlwithicon;
		$downloadPath = $this->addWaterMarkPrivate($videoMainPath,$watermarkPath,$usernameWater,$basenamepath);
	...
	public function forgotPass(){
	    ...
	    if(!empty($check)){
				...
				if($type == 'phone'){
					...
					$created = date('Y-m-d+H:s:i');
					$url = "https://api.msg91.com/api/sendhttp.php?authkey=339739Aqv4rSuggd5f44f645P1&mobiles=$mobile&country=91&message=$mess&sender=Cinemaflix&route=4";
					$ch = curl_init();
					curl_setopt($ch,CURLOPT_URL,$url);
					curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
					$output=curl_exec($ch);
					...
					$url = "http://Cinemaflix.in/SendEmail/sendEmail.php?otp=$otp&email=$emailPhone";
						$ch = curl_init();
						curl_setopt($ch,CURLOPT_URL,$url);
						curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
						$output=curl_exec($ch);
	...
	public function userNotification(){
		...
		$lists = $this->Common_Model->userNotification($this->input->post('userId'),$startLimit);
		if(!empty($lists)){
  ...
  //       }
	...
	public function userInfo(){
		...
		$videoTotalCount = $this->db->get_where('userVideos', array('userId' => $this->input->post('userId')))->num_rows();
		if(!empty($videoTotalCount)){
			$finalData['videoCount'] = (string)$videoTotalCount;
		}
		else{
			$finalData['videoCount'] = '0';
		}
		$getFollowers = $this->db->get_where('userFollow', array('followingUserId' => $this->input->post('userId'),'status' => '1'))->num_rows();
		if(!empty($getFollowers)){
			$finalData['followers'] = (string)$getFollowers;
		}
		else{
			$finalData['followers'] = '0';
		}

		$likeCount = $this->db->get_where('videoLikeOrUnlike', array('ownerId' => $mmUserId,'status'=> '1'))->num_rows();
		if(!empty($likeCount)){
			$finalData['likes'] = (string)$likeCount;
		}
		else{
			$finalData['likes'] = '0';
		}
	...
	public function getLikeVideo(){
		...
		if(!empty($getVideoIds)){
			...
			$message['message'] = 'List found successfully';
			foreach($getVideoIds as $lists){
				$lists['soundTitle'] = 'Video uploaded by '.$lists['username'];
                ...
                $checkFollow = $this->db->get_where('userFollow', array('userId' => $this->input->post('userId'),'followingUserId' => $lists['userId']))->row_array();
                if(!empty($checkFollow)){
                    $lists['followStatus'] = $checkFollow['status'];
                }
                else{
	...
	public function likeAndDislikeVideo(){
		...
		$idtest = array(
			'video' => $this->input->post('userId'),
			'videoId' => $this->input->post('videoId'),
			'ownerId' => $this->input->post('ownerId'),
		);
		$this->db->insert('videoTesting',$idtest);
 	 ...
 	 if($status == '1'){
 		 ...
 		 $regId = $UserDetails['reg_id'];
 			if($this->input->post('userId') != $this->input->post('ownerId') && $notificatinSendStatus == 1){
 			 if($UserDetails['likeNotifaction'] == '1'){
	...
	public function hashTest(){
		// $hastage = $this->input->post('hashL');
		// $string = str_replace(" ",",",$hastage);
		// $expmm = explode(',',$string);
		// foreach($expmm as $expmmss){
			// $checkU = substr($expmmss,0,1);
			// if($checkU == '#'){
				// $finalHashValue[] = $expmmss;
			// }
			// else{
				// $finalHashValue[] = '#'.$expmmss;
			// }
		// }
		// $list = implode(',',$finalHashValue);
		// $finalList =  str_replace(" ",",",$list);

	}

	public function hastTagIds($hastage,$userId,$videoId){
		//$string = $this->input->post('hashL');
		$string = str_replace(" ",",",$hastage);
		$hasTag = $string;
		$exp = explode(',',$hasTag);
		foreach($exp as $expss){
			$checkU = substr($expss,0,1);
			if($checkU == '#'){
				$exps = $expss;
			}
			else{
			...
			$videoHastagList['userId'] = $userId;
			$videoHastagList['hashTag'] = $exps;
			$videoHastagList['videoId'] = $videoId;
			$videoHastagList['created'] = date('Y-m-d H:i:s');
			$this->db->insert('videoHashtagList',$videoHastagList);
	...
	public function uploadVideosOld(){
		require APPPATH.'/libraries/vendor/autoload.php';
	...
	//	@unlink($video_file);
	 ...
	 function addWaterMarkPrivatOld($video_file,$watermark,$username,$basename_file,$userpic,$description){
	...
	// 	return $video_merged;
	// }
	function addtextImageOLD($gifpath,$watermarkpath,$textwrite){
		$gmagick = new Gmagick($watermarkpath);
		// echo "23";
		$gmagick = $gmagick->coalesceImages();
		// Create a GmagickDraw object
	...
	public function getVideo(){
    	$startLimit = $this->input->post('startLimit');
			if(!empty($this->input->post('anuradha'))){
				$videoTypedd = $this->input->post('anuradha');
			}
			else{
				$videoTypedd = 0;
			}
		...
		if($this->input->post('videoType') == 'following'){
			$list =  $this->db->query("SELECT sounds.title as soundTitle,sounds.id as soundId,users.username,users.followerCount as followers,users.image,userVideos.id, userVideos.userId,userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDownloads, userVideos.allowDuetReact,userVideos.viewVideo,userVideos.likeCount,userVideos.commentCount,userVideos.downloadPath,userVideos.videolength FROM `userVideos` left join userFollow on userFollow.followingUserId = userVideos.userId left join users on users.id = userVideos.userId left join sounds on sounds.id = userVideos.soundId where userFollow.userId = $userId and userVideos.status = '1' and userVideos.videoType = $videoTypedd and userFollow.status = '1'  group by userVideos.userId order by RAND() LIMIT  10 ")->result_array();
		}
		else{
			...
			$list =  $this->db->query("SELECT users.username,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDuetReact,userVideos.allowDownloads,userVideos.viewVideo,userVideos.likeCount,userVideos.commentCount,userVideos.downloadPath,userVideos.videolength FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.viewVideo = 0 and userVideos.videoType = $videoTypedd and userVideos.status = '1' and userVideos.userId NOT IN (select blockUserId from blockUser where userId = '$userId' )  group by userVideos.userId ORDER BY RAND() LIMIT 10")->result_array();
		}
	...
	public function checkPhoneAndEmail(){
		...
		//select * from users where phone ="7087772978" || email="admin@gmail.com" || username="@admin"
		$checkData =  $this->db->query("select * from users where phone ='$phone' || email='$email' || username='$username'")->result_array();
		//$checkEmail = $this->db->get_where('users',array($type => $this->input->post('emailPhone')))->row_array();
		if(empty($checkData)){
			$otp = rand(1000,9999);
			...
			//
			//
			// $url = "http://Cinemaflix.in/SendEmail/sendEmail.php?otp=$otp&email=$email";
			// $ch = curl_init();
			// curl_setopt($ch,CURLOPT_URL,$url);
			// curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
			// $output=curl_exec($ch);
			...
			$message['success'] = '0';
			$message['message'] = 'Information is alerdy exist';
		}
		echo json_encode($message);
	}
	...
	public function imageVideo(){
		...
		if(!empty($_FILES['image']['name'])){
        	...
        	$url = $upload->get('ObjectURL');
        	if(!empty($url)){
            	$details['image'] = 'http://d29vf2dbcha7zq.cloudfront.net/'.$_FILES['image']['name'];;
        	}
        	else{
            	$details['image'] = '';
        	}
	...
	public function login(){
		...
		$data = $this->db->query("select * from users where username='$username'  and password = '$password'")->row_array();
		if(!empty($data)){
			$datas = array('onlineStatus'=>1,'reg_id' => $this->input->post('reg_id'),'device_type' => $this->input->post('device_type'));
	...
	public function checkUserPhone(){
 	...
 	public function matchVerificationTokenOld(){
		if($this->input->post()){
...
// 			}
  ...
  public function enquiry(){
	...
	public function videoSearchApi($search,$userId){
		...
		if(!empty($userLists)){
			foreach($userLists as $userList){
				...
				$likeCount = $this->db->get_where('videoLikeOrUnlike', array('videoId' => $userList['id'],'status'=> '1'))->num_rows();
				if(!empty($likeCount)){
					$userList['likeCount'] = (string)$likeCount;
				}
				else{
					$userList['likeCount'] = '0';
				}



				$checkFollow = $this->db->get_where('userFollow', array('userId' => $this->input->post('userId'),'followingUserId' => $userList['userId']))->row_array();
				if(!empty($checkFollow)){
					$userList['followStatus'] = $checkFollow['status'];
				}
				else{
					$userList['followStatus'] = '0';
				}
	...
	public function userHashTagApi($search,$userId){
		...
		if(!empty($getHashtag)){
			foreach($getHashtag as $getHashtags){
				$countHash = $this->db->get_where('videoHashtagList',array('hashTag' => $getHashtags['hashtag']))->num_rows();
				if(!empty($countHash)){
					$getHashtags['videoCount'] = (string)$countHash;
				}
				else{
					$getHashtags['videoCount'] = '0';
				}
				$finalHashTag[] = $getHashtags;
			}
		}
		else{
			$finalHashTag = [];
	...
	public function soundVideos(){
		...
		$getVideo = $this->db->order_by('id','asc')->get_where('userVideos',array('soundId' => $this->input->post('soundId'),'status !=' => '2'))->row_array();
		if(!empty($getVideo)){
			...
			$userInfo['soundTitle'] = 'Video uploaded by '.$userDetails['username'];
			...
			$userInfo['viewVideo'] = $getVideo['viewCount'];
            ...
            $checkFollow = $this->db->get_where('userFollow', array('userId' => $this->input->post('userId'),'followingUserId' => $getVideo['userId']))->row_array();
            if(!empty($checkFollow)){
                $userInfo['followStatus'] = $checkFollow['status'];
            }
            else{
			...
			$list =  $this->db->query("SELECT sounds.title as soundTitle,sounds.id as soundId,sounds.soundCount as videoCount,sounds.sound as soundPath,sounds.type as soundType, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads, userVideos.viewVideo, userVideos.viewCount, userVideos.likeCount, userVideos.commentCount,userVideos.viewCount,userVideos.allowDuetReact FROM `userVideos` left join sounds on sounds.id = userVideos.soundId where userVideos.id !=  $videoId and userVideos.soundId = $soundId and userVideos.status != '2' and userVideos.userId NOT IN (select blockUserId from blockUser where userId = $userId  ) ORDER BY userVideos.id ASC LIMIT   $startLimit,20")->result_array();
			if(!empty($list)){
				$fcountHasth = $this->db->query("SELECT  * FROM `userVideos` where soundId = $soundId")->num_rows();
				$userInfo['countVideo'] = (string)$fcountHasth;
				foreach($list as $lists){
					...
					$lists['viewVideo'] = $lists['viewCount'];
					...
					$lists['countVideo'] = (string)$fcountHasth;
	...
	public function hashTagVideos(){
		$idtest['title'] = $this->input->post('userId');
		$idtest['sdfsdf'] = $this->input->post('hashtagId');
		$this->db->insert('testing',$idtest);
		...
		if(!empty($getVideo)){
			...
			$userId = $this->input->post('userId');
			$startLimit = $this->input->post('startLimit');
			$list =  $this->db->query("SELECT sounds.title as soundTitle,sounds.id as soundId,sounds.soundCount as videoCount,sounds.sound as soundPath, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.viewCount,userVideos.likeCount, userVideos.commentCount,userVideos.viewCount,userVideos.allowDuetReact,userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId where userVideos.id !=  $videoId and userVideos.hashTag Like '%$hashtagId%' and userVideos.status != '2' and userVideos.userId NOT IN (select blockUserId from blockUser where userId = $userId  ) ORDER BY userVideos.id ASC LIMIT   $startLimit,20")->result_array();

			if(!empty($list)){
				$userInfo['countVideo'] = (string)$getHashTag['videoCount'];
				foreach($list as $lists){
					if(!empty($lists['downloadPath'])){
						$lists['downloadPath'] = $lists['downloadPath'];
					}
					...
					$lists['countVideo'] = (string)$getHashTag['videoCount'];
			...
			//$array['hashtagVideo'] = $finalSoundList;
			$message['success'] = '1';
			$message['message'] = 'List found successfully';
			$message['details'] = $array;
		}
	...
	public function sendMessage(){
		...
		if(empty($get_sender_data)){
			...
			if($this->input->post('messageType')=='1'){
				...
				$url = $upload->get('ObjectURL');
				if(!empty($url)){
					$data['image'] = 'http://d29vf2dbcha7zq.cloudfront.net/'.$_FILES['image']['name'];;
				}
				else{
					$data['image'] = '';
				}
	...
	public function singleVideo(){
		...
		$videoId = $this->input->post('videoId');
		$lists =  $this->db->query("SELECT users.username,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDuetReact, userVideos.allowDownloads, userVideos.viewVideo,userVideos.videoType, userVideos.likeCount, userVideos.commentCount, userVideos.downloadPath,userVideos.videolength FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.status != '2' and userVideos.id = $videoId ")->row_array();
		if(!empty($lists)){
			$lists['soundTitle'] = 'Video uploaded by '.$lists['username'];
			$message['success'] = '1';
			$message['message'] = 'List Found Successfully';
	...
	public function userReportVideo(){
		...
		$data['report'] = $this->input->post('report');
		$data['created'] = date('Y-m-d H:i:s');
		$insert = $this->db->insert('userReportVideo',$data);
		$message['success'] = '1';
		$message['message'] = 'Report send Successfully';
		echo json_encode($message);
	}
	...
	public function videoTesting(){
			$getAdminlimit = $this->db->get_where('adminVideoLimit',array('id' => 1))->row_array();
			if(!empty($getAdminlimit)){
				$finalAdminLimit = $getAdminlimit['setLimit'];
			}
			else{
				$finalAdminLimit = 10;
			}
		...
		if($this->input->post('videoType') == 'following'){
			$list =  $this->db->query("SELECT sounds.title as soundTitle,sounds.id as soundId,users.username,users.image,userVideos.id, userVideos.userId,userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDownloads, userVideos.allowDuetReact,userVideos.viewVideo,userVideos.commentCount,userVideos.downloadPath,userVideos.videolength,userVideos.videoType FROM `userVideos` left join userFollow on userFollow.followingUserId = userVideos.userId left join users on users.id = userVideos.userId left join sounds on sounds.id = userVideos.soundId where userFollow.userId = $userId and userVideos.status != '2'  and userFollow.status = '1' and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' and userViewType = 1 )  order by RAND() LIMIT  $finalAdminLimit")->result_array();
		}
		else{
			// $list =  $this->db->query("SELECT users.username,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDuetReact,userVideos.allowDownloads,userVideos.viewVideo,userVideos.likeCount,userVideos.commentCount,userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.viewVideo = 0 and userVideos.userId NOT IN (select blockUserId from blockUser where userId = '$userId' ) and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' ) ORDER BY userVideos.viewCount desc,userVideos.likeCount desc,userVideos.commentCount LIMIT $startLimit , 5")->result_array();
			$list =  $this->db->query("SELECT users.username,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDuetReact,userVideos.allowDownloads,userVideos.viewVideo,userVideos.commentCount,userVideos.downloadPath,userVideos.videolength,userVideos.videoType FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.status = '1' and userVideos.videoType = $videoTypedd and userVideos.userId NOT IN (select blockUserId from blockUser where userId = '$userId'  ) and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' and userViewType = 1 )    ORDER BY RAND() LIMIT  $finalAdminLimit")->result_array();
		}
		...
		$count = count($list);
			...
			foreach($list as $lists){
				...
				$viewVideoInsert['userViewType'] = 1;
				...
				$followersCountTotal = $this->db->get_where('userFollow', array('followingUserId' => $lists['userId'],'status' => '1'))->num_rows();
				if(!empty($followersCountTotal)){
					$lists['followers'] = (string)$followersCountTotal;
				}
				else{
					$lists['followers'] = '0';
				}
				...
				$lists['advertisementStatus'] = '0';


				$mainVideo[] = $lists;
			}

			$adminVideo =  $this->db->query("select * from adminVideos where status = 'Approved' and id NOT IN (select videoId from  adminVideoView where userId = '$userId' and userViewType = 1) order by id asc  limit 1")->row_array();

			if(!empty($adminVideo)){
				$getCount1 = $this->db->get_where('adminVideos',array('id' => $adminVideo['id']))->row_array();
				$upCount1['viewCount'] = $getCount1['viewCount'] + 1;
				$this->Common_Model->update('adminVideos',$upCount1,'id',$adminVideo['id']);

				$viewVideoInsert1['userId'] = $this->input->post('userId');
				$viewVideoInsert1['videoId'] = $adminVideo['id'];
				$viewVideoInsert1['userViewType'] = 1;
				$this->db->insert('adminVideoView',$viewVideoInsert1);

				$countAdminVideoView = $this->db->get_where('adminVideoView',array('userViewType' => 1 ,'userId'=>$this->input->post('userId') ))->num_rows();
				$countAdminVideo = $this->db->get_where('adminVideos',array('status' => 'Approved' ))->num_rows();
				if($countAdminVideoView == $countAdminVideo || $countAdminVideoView > $countAdminVideo){
					$upAdminVideoView['userViewType'] = 0;
					$this->db->update("adminVideoView",$upAdminVideoView,array('userId'=>$this->input->post('userId')));
				}
				$adminVideo['likeStatus'] = false;
				$adminVideo['followStatus'] = '0';
				$adminVideo['likeCount'] = '0';
				$adminVideo['commentCount'] = '0';
				$adminVideo['advertisementStatus'] = '1';
				$fianlAdminVideo[] = $adminVideo;
				$finalListSound = array_merge($mainVideo,$fianlAdminVideo);
			}
			else{
				$finalListSound = $mainVideo;
			}
			$message['success'] = '1';
			$message['message'] = 'List found successfully';
			...
			if($this->input->post('videoType') == 'following'){
				$list =  $this->db->query("SELECT sounds.title as soundTitle,sounds.id as soundId,users.username,users.image,userVideos.id, userVideos.userId,userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDownloads, userVideos.allowDuetReact,userVideos.viewVideo,userVideos.commentCount,userVideos.downloadPath,userVideos.videolength,userVideos.videoType FROM `userVideos` left join userFollow on userFollow.followingUserId = userVideos.userId left join users on users.id = userVideos.userId left join sounds on sounds.id = userVideos.soundId where userFollow.userId = $userId and userVideos.status = '1'  and userFollow.status = '1' and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' and userViewType = 1 )  order by RAND() LIMIT  $finalAdminLimit")->result_array();
			}
			else{
				$list =  $this->db->query("SELECT users.username,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDuetReact,userVideos.allowDownloads,userVideos.viewVideo,userVideos.commentCount,userVideos.downloadPath,userVideos.videolength,userVideos.videoType FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.status = '1' and userVideos.videoType = $videoTypedd and userVideos.userId NOT IN (select blockUserId from blockUser where userId = '$userId'  ) and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' and userViewType = 1 )    ORDER BY RAND() LIMIT  $finalAdminLimit")->result_array();
			}
	...
	public function adminVideo(){
		...
		if($this->input->post('type') == '1'){
			$getCount = $this->db->get_where('adminVideos',array('id' => $this->input->post('videoId')))->row_array();
			$upCount['viewCount'] = $getCount['viewCount'] + 1;
			$update = $this->Common_Model->update('adminVideos',$upCount,'id',$this->input->post('videoId'));
			if(!empty($update)){
				$data['userId'] = $this->input->post('userId');
				$data['videoId'] = $this->input->post('videoId');
				$data['created'] = date('Y:m:d H:i:s');
				$this->db->insert('adminVideoView',$data);
				$message['success'] = '1';
				$message['message'] = 'View count added successfully';
			}
			else{
				$message['success'] = '0';
				$message['message'] = 'Please try after some time';
			...
			$getCount = $this->db->get_where('adminVideos',array('id' => $this->input->post('videoId')))->row_array();
			$upCount['hitCount'] = $getCount['hitCount'] + 1;
			$update = $this->Common_Model->update('adminVideos',$upCount,'id',$this->input->post('videoId'));
			if(!empty($update)){
				$data['userId'] = $this->input->post('userId');
				$data['videoId'] = $this->input->post('videoId');
				$data['created'] = date('Y:m:d H:i:s');
				$this->db->insert('adminVideoHitCount',$data);
				$message['success'] = '1';
				$message['message'] = 'Hit count added successfully';
			}
			else{
				$message['success'] = '0';
				$message['message'] = 'Please try after some time';
	...
	public function uploadSingleVideo(){
		...
		$url = $upload->get('ObjectURL');
		if(!empty($url)){
			$data['video'] = 'http://d29vf2dbcha7zq.cloudfront.net/'.$_FILES['video']['name'];;
		}
		else{
			$data['video'] = '';
		}
		$insert = $this->db->insert('videoTesting',$data);
		$list = $this->db->order_by('id','desc')->get_where('videoTesting')->result_array();
		$message['success'] = '1';
		$messgae['message'] = 'Video upload succssfully';
		$message['details'] = 	$list;
			echo json_encode($message);
	}


	public function updateVideoInformation(){
		$data['allowDownloads']  = $this->input->post('allowDownloads');
	  $data['description'] = $this->input->post('description');
	  $data['allowComment'] = $this->input->post('allowComment');
		$update = $this->Common_Model->update('userVideos',$data,'id',$this->input->post('videoId'));
		if(!empty($update)){
			$message['success'] = '1';
