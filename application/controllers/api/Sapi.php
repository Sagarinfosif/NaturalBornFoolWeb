  ...
  //       }
	...
	public function getLikeVideo(){
		...
		if(!empty($getVideoIds)){
			...
			foreach($getVideoIds as $lists){
				...
				$followStatus = $this->db->get_where('userFollow', array('followingUserId' =>$lists['userId'],'userId'=> $this->input->post('userId'),'status'=> '1'))->row_array();
				if(!empty($likeStatus)){
				...
				if(!empty($lists['categoryId'])){
					$catTi = $this->db->get_where('category',array('id' => $lists['categoryId']))->row_array();
					$lists['categoryId'] = $catTi['id'];
					$lists['categoryTitle'] = $catTi['title'];
				}
				else{
					$lists['categoryId'] = '1';
					$lists['categoryTitle'] = 'omninos';
				}
	...
	public function myVideoList(){
		...
		if(!empty($videoList)){
			foreach($videoList as $lists){
				...
				$followStatus = $this->db->get_where('userFollow', array('followingUserId' =>$lists['userId'],'userId'=> $this->input->post('login'),'status'=> '1'))->row_array();
				if(!empty($likeStatus)){
					$lists['followStatus'] = '1';
				}
				else{
	...
	public function getVideo(){
		...
		if($this->input->post('videoType') == 'following'){
			...
			$list =  $this->db->query("SELECT users.username,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount,userVideos.downloadCount, userVideos.allowComment, userVideos.allowDuetReact,userVideos.allowDownloads,userVideos.viewVideo,userVideos.likeCount,userVideos.commentCount,userVideos.downloadPath,userVideos.categoryId FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.viewVideo = 0 and userVideos.userId NOT IN (select blockUserId from blockUser where userId = '$userId' ) and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' ) ORDER BY RAND() LIMIT $startLimit , 5")->result_array();
		}
  ...
  public function enquiry(){
	...
	public function videoSearchApi($search,$userId){
		...
		if(!empty($userLists)){
			foreach($userLists as $userList){
				...
				$followStatus = $this->db->get_where('userFollow', array('followingUserId' =>$userList['userId'],'userId'=> $userId,'status'=> '1'))->row_array();
				if(!empty($likeStatus)){
					$userList['followStatus'] = '1';
				}
				else{
				...
				if(!empty($userList['categoryId'])){
					$catTi = $this->db->get_where('category',array('id' => $userList['categoryId']))->row_array();
					$userList['categoryId'] = $catTi['id'];
					$userList['categoryTitle'] = $catTi['title'];
				}
				else{
					$userList['categoryId'] = '1';
					$userList['categoryTitle'] = 'omninos';
				}
	...
	public function soundVideos(){
		...
		if(!empty($getVideo)){
			...
			if(!empty($getSound['userId'])){
				...
				$followStatus = $this->db->get_where('userFollow', array('followingUserId' =>$userDetails['id'],'userId'=> $this->input->post('userId'),'status'=> '1'))->row_array();
				if(!empty($likeStatus)){
			...
			$userInfo['categoryId'] = $getVideo['categoryId'];
			$userInfo['downloadCount'] = $getVideo['downloadCount'];
			...
			if(!empty($getVideo['categoryId'])){
				$catTi = $this->db->get_where('category',array('id' => $getVideo['categoryId']))->row_array();
				$userInfo['categoryId'] = $catTi['id'];
				$userInfo['categoryTitle'] = $catTi['title'];
			}
			else{
				$userInfo['categoryId'] = '1';
				$userInfo['categoryTitle'] = 'omninos';
			}
	...
	public function hashTagVideos(){
		...
		if(!empty($getVideo)){
			...
			$userId = $this->input->post('userId');
			$startLimit = $this->input->post('startLimit');
			$list =  $this->db->query("SELECT sounds.title as soundTitle,sounds.id as soundId,sounds.soundCount as videoCount,sounds.sound as soundPath, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.viewCount,userVideos.likeCount, userVideos.commentCount,userVideos.viewCount,userVideos.allowDuetReact,userVideos.downloadPath,userVideos.downloadCount,userVideos.categoryId FROM `userVideos` left join sounds on sounds.id = userVideos.soundId where userVideos.id !=  $videoId and userVideos.hashTag Like '%$hashtagId%' and userVideos.userId NOT IN (select blockUserId from blockUser where userId = $userId  ) ORDER BY userVideos.id ASC LIMIT   $startLimit,20")->result_array();
	...
	public function singleVideo(){
		...
		$videoId = $this->input->post('videoId');
		$lists =  $this->db->query("SELECT users.username,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDuetReact, userVideos.allowDownloads, userVideos.viewVideo, userVideos.likeCount, userVideos.commentCount, userVideos.downloadPath,userVideos.downloadCount, userVideos.categoryId FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.viewVideo = 0 and userVideos.id = $videoId ")->row_array();
		if(!empty($lists)){
			$message['success'] = '1';
			$message['message'] = 'List Found Successfully';
			if(!empty($lists['downloadPath'])){
	...
	public function downloadCount(){
		$getCount =  $this->db->get_where('userVideos',array('id' => $this->input->post('videoId')))->row_array();
		$updateCount['downloadCount'] = $getCount['downloadCount'] + 1;
		$this->Common_Model->update('userVideos',$updateCount,'id',$this->input->post('videoId'));
		$message['success'] = '1';
		$message['message'] = 'count added successfully';
		echo json_encode($message);
	}

	public function hastagVideoList(){
		$search = '';
		$userId = $this->input->post('userId');
		$getHashtag =  $this->Common_Model->gethashTag($search);
		if(!empty($getHashtag)){
			...
			foreach($getHashtag as $getHashtags){
				$hashtagId = $getHashtags['id'];
				$list =  $this->db->query("SELECT sounds.title as soundTitle,sounds.id as soundId,sounds.soundCount as videoCount,sounds.sound as soundPath,sounds.type as soundType, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.viewCount,userVideos.likeCount, userVideos.commentCount,userVideos.viewCount,userVideos.allowDuetReact FROM `userVideos` left join sounds on sounds.id = userVideos.soundId where  userVideos.hashTag Like '%$hashtagId%' and userVideos.userId NOT IN (select blockUserId from blockUser where userId = $userId  ) ORDER BY userVideos.id ASC LIMIT  0 , 10")->result_array();
				if(!empty($list)){
					foreach($list as $lists){
						$userDetails = $this->db->get_where('users',array('id' => $lists['userId']))->row_array();
						$lists['name'] = $userDetails['name'];
						...
						if(!empty($userDetails['image'])){
							if (filter_var($userDetails['image'], FILTER_VALIDATE_URL)) {
								...
								$lists['image'] = base_url().$userDetails['image'];
							}
						}
						else{
							$lists['image'] = base_url().'uploads/no_image_available.png';
						...
						$lists['soundPath'] = base_url().$lists['soundPath'];
						$getHashtags['videoList'][] = $lists;
					}
				}
				else{
					$getHashtags['videoList'] = [];
				}
				$message['details'][] = $getHashtags;
			}
		}
		else{
			$message['success'] = '0';
	...
	public function popular(){
		$userId = $this->input->post('userId');
		$list =  $this->db->query("SELECT users.id as userId,users.name,users.username,users.followerCount as followers,users.image,userProfileInformation.likes FROM users left join userProfileInformation on userProfileInformation.userId = users.id where users.id NOT IN (select blockUserId from blockUser where userId = $userId ) ORDER BY users.followerCount desc,userProfileInformation.likes desc")->result_array();
		if(!empty($list)){
			$message['success'] = '1';
			$message['message'] = 'List found successfully';
			foreach($list as $lists){
				if(!empty($lists['image'])){
					if (filter_var($lists['image'], FILTER_VALIDATE_URL)) {
	...
	public function category(){
		$getCategory =  $this->Common_Model->getCategory($this->input->post('search'));
		if(!empty($getCategory)){
			$message['success'] = '1';
			$message['message'] = 'List found successfully';
			foreach($getCategory as $getCategorys){
				$getCategorys['image'] = base_url().$getCategorys['image'];
				$message['details'][] = $getCategorys;
			}
		}
		else{
			$message['success'] = '0';
	...
	$list =  $this->db->query("SELECT category.title as categoryTitle, sounds.title as soundTitle,sounds.id as soundId,sounds.soundCount as videoCount,sounds.sound as soundPath,sounds.type as soundType, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.viewCount,userVideos.likeCount, userVideos.commentCount,userVideos.viewCount,userVideos.allowDuetReact,userVideos.downloadCount,userVideos.categoryId FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join category on category.id = userVideos.categoryId where userVideos.categoryId = $categoryId  ORDER BY userVideos.id ASC LIMIT  0 , 10")->result_array();
	...
	if(!empty($list)){
		foreach($list as $lists){
			...
			$lists['videoPath'] = base_url().$lists['videoPath'];
			$lists['soundPath'] = base_url().$lists['soundPath'];
			$finalSoundList[] = $lists;
		}
	}
	...
	public function getNews(){
		$list = $this->db->get_where('news')->result_array();
		if(!empty($list)){
			$message['success'] = '1';
			$message['message'] = 'List found successfully';
			foreach($list as $lists){
				$lists['image'] = base_url().$lists['image'];
				$lists['newsTime'] = $this->getTime($lists['created']);
				$message['details'][] = $lists;
			}
		}
		else{
