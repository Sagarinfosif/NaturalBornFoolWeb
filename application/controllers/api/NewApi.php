...
class NewApi extends CI_Controller {
	...
	public function funtroos(){
		$talvido = $this->db->order_by('id','asc')->get_where('badges')->result_array();
		if(!empty($talvido)){
			$userBadge = $this->db->get_where('users',array('id' => $this->input->post('userId')))->row_array();
			$userBadgeid = $userBadge['badge'];
			$message['success'] = '1';
			$message['message'] = 'List found successfully';
			$message['color'] = '#f18947';
			foreach($talvido as $talvidos){
				$talvidoss['id'] = $talvidos['id'];
				$talvidoss['color'] = $talvidos['color'];
				$talvidoss['image'] = $talvidos['image'];
				$talvidoss['title'] = $talvidos['title'];
				$talvidoss['description'] = '';
				if($talvidos['id'] == $userBadgeid ){
					$talvidoss['status'] = true;
				}
				else{
					$talvidoss['status'] = false;
				}
				$message['details'][] = $talvidoss;
			}
		}
		else{
			$message['success'] = '0';
  ...
  //       }
	...
	public function uploadVideos(){
		...
		$defaultUserPic = "uploads/dummy.png";
		...
		$dir_name = "/var/www/html/admin/application/controllers/";
		$uploadPath = "uploads/";
		$duetUrl = "";
...
// 			}
  ...
  public function enquiry(){
	...
	public function singleVideo(){
		...
		$videoId = $this->input->post('videoId');
		$lists =  $this->db->query("SELECT users.username,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.viewCount, userVideos.allowComment, userVideos.allowDuetReact, userVideos.allowDownloads, userVideos.viewVideo, userVideos.likeCount, userVideos.commentCount, userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.viewVideo = 0 and userVideos.id = $videoId ")->row_array();
		if(!empty($lists)){
			$message['success'] = '1';
			$message['message'] = 'List Found Successfully';
			if(!empty($lists['downloadPath'])){
